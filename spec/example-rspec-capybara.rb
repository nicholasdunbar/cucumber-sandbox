...
#initialize environment variables from the cucumber set-up
if File.file?('features/support/.env.dev')
  puts 'Config: .env.dev'
  DotEnv.get_environment('features/support/.env.dev')
else
  puts 'Config: .env'
  DotEnv.get_environment('features/support/.env')
end
